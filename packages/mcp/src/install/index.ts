...
export async function runInstall(argv: string[]): Promise<void> {
  ...
  const env =
    parsed.apiUrl !== undefined && parsed.apiUrl.length > 0
      ? { NURAVIEW_API_URL: parsed.apiUrl }
      : undefined;
...
function printInstallHelp(): void {
  console.log(`nuraview-mcp install — register NuraView in an MCP client config

Usage:
  nuraview-mcp install [options]
...
Options:
  ...
  --name <string>     MCP server key under mcpServers (default: nuraview)
  --api-url <url>     Set NURAVIEW_API_URL in the generated entry (optional)
  -y, --yes           Overwrite existing entry without prompting
  -h, --help          Show this help

Examples:
  npm install -g @nuraview/mcp
  nuraview-mcp install

  nuraview-mcp install --target cursor-user -y
  nuraview-mcp install --target custom --output /path/to/mcp.json -y
`);
}
