  ...
  .post(
    ...
    async (c) => {
      ...
      const apiBaseUrl = normalizeApiServerUrl(
        process.env.NURAVIEW_API_URL || new URL(c.req.url).origin,
      );
      return c.json({
        id: asset.id,
...
/**
 * Files on a task. Rendered as their own panel in the UI — the Trello import
 * used to flatten these into the description, which is what made an imported
 * card read as a wall of text.
 */
task.get("/:id/attachments", async (c) => {
  const rows = await db
    .select()
    .from(taskAttachmentTable)
    .where(eq(taskAttachmentTable.taskId, c.req.param("id")))
    .orderBy(taskAttachmentTable.createdAt);

  return c.json({ items: rows });
});

/* ------------------------------------------------------------ card link --
 * One card as a standalone page, for the people already on that project.
 *
 * There is no enable/disable and no token: the link is just the card id, and
 * every request is authorised against project_member. What used to be here —
 * POST /:id/share and /:id/share/rotate, minting an anonymous public URL — is
 * gone; see card-link.ts for why.
 */
task.get("/:id/card", workspaceAccess.fromTask(), async (c) => {
  return c.json(await getTaskCard(c.req.param("id"), c.get("userId")));
});
