...
export function getStorageClient() {
  return getClient(getStorageConfig());
}
 ...
 *
 * The presigned-URL path exists for browser uploads, where the file never
 * touches the API. Server-side imports (Trello attachments) already have the
 * bytes, so a direct PutObject avoids a pointless round trip through a
 * presigned URL back to ourselves.
 *
 * Returns the storage KEY, not a URL — these objects are private and are served
 * through /api/asset/:id, which does its own access check.
 */
