...
/**
 * Instance branding. Every field is always present — getBrand() falls back to
 * NuraView's own values rather than omitting keys, so the client never has to
 * write `brand?.name ?? "..."` and cannot accidentally render an empty title.
 */
export const brandSchema = v.object({
  ...
  marketingUrl: v.string(),
  supportEmail: v.nullable(v.string()),
  legalName: v.string(),
  ...
  signature: v.object({
    ...
    linkedinLabel: v.nullable(v.string()),
    websiteUrl: v.string(),
    websiteLabel: v.string(),
    legalLine: v.nullable(v.string()),
    addressLine: v.nullable(v.string()),
  }),
});
