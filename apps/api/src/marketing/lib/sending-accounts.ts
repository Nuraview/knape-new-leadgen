...
export function getDefaultSendingAccount(): SendingAccount {
  const [first] = getSendingAccounts();
  if (!first) {
    // The list is code-defined and cannot legitimately be empty. If it ever is,
    // sending with no identity would silently go out as the wrong sender.
    throw new Error("No sending accounts are configured");
  }
  return first;
}
