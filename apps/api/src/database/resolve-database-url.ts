const LOCAL_FALLBACK_CONNECTION_STRING = "postgresql://localhost:5432/nuraview";
...
export function resolveDatabaseConfig(): ResolvedDatabaseConfig {
  ...
  if (getDerivationSignal()) {
    ...
    const username = process.env.POSTGRES_USER || "nuraview";
    const password = encodeURIComponent(process.env.POSTGRES_PASSWORD);
    const host = process.env.POSTGRES_HOST || "postgres";
    const port = process.env.POSTGRES_PORT || "5432";
    const database = process.env.POSTGRES_DB || "nuraview";
