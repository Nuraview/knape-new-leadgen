...
export const userTable = pgTable("user", {
  ...
  /**
   * Forced password rotation. Set when an operator provisions or resets an
   * account with a temporary password — the holder must choose their own
   * before they can use the app, so the temporary one never becomes the real
   * one and whoever issued it never learns the replacement.
   */
  mustChangePassword: boolean("must_change_password").default(false).notNull(),
});

/** better-auth twoFactor plugin storage. Column names are the plugin's. */
export const twoFactorTable = pgTable(
  "two_factor",
  {
    ...
    /**
     * False until a code from the authenticator has actually been checked.
     * The row is created when enrolment STARTS, so treating it as active on
     * creation would lock out anyone whose QR scan silently failed.
     */
    verified: boolean("verified").notNull().default(false),
    /**
     * Lockout state, both managed by the plugin — nothing in this app writes
     * them. Present because better-auth's declared schema has six fields and a
     * missing one throws at the adapter, not at typecheck.
     */
...
export const projectTable = pgTable(
  "project",
  {
    ...
    description: text("description"),
    createdAt: timestamp("created_at", { mode: "date" }).defaultNow().notNull(),
    // No is_public: a board is internal. Every read goes through the
    // authenticated project routes and canOpenProject. Dropped in 0049.
...
export const taskTable = pgTable(
  "task",
  {
    ...
    description: text("description"),
    // No public_share_id / public_shared_at here on purpose: card links are
    // authorised per request against project_member (task/card-link.ts), not
    // held as a bearer token on the row. Dropped in 0048.
...
/**
 * Explicit per-user entitlement, covering the two product areas that live in
 * this one app.
 *
 * Access used to be inferred from workspace role (owner|admin => CRM). That
 * coupled two unrelated things: how much of the PROJECT BOARDS you administer,
 * and whether you may read the lead pipeline. It also meant any path that
 * granted an owner row — such as creating a workspace — silently granted the
 * whole CRM.
 *
 * `level` (lead pipeline):
 *   full         — leads list, activity, writes; everything ported so far
 *   leads_kanban — read-only leads, kanban only; no writes, no activity
 *
 * `canAccessProjects` (project boards): a lead-gen employee gets the kanban and
 * nothing else — no boards, no member list, no invitations.
 *
 * NO ROW means: no CRM, projects allowed. That is the projects-only employee,
 * and it is why the column defaults to true. Instance owners/admins are treated
 * as `full` CRM without needing a row.
 */
export const userAccessTable = pgTable("user_access", {
  ...
  /**
   * Pre-selected task for people whose work is one repeating thing. When set,
   * the clock attributes to it automatically and the picker collapses to that
   * single entry — VK, about a lead-gen employee: "one task alone, which needs
   * to be pre-selected".
   */
  pinnedTaskId: text("pinned_task_id"),
 ...
 *
 * `endedAt` NULL = still on the clock. This is NOT activity surveillance —
 * there is no screenshot or idle detection, by explicit decision; it records
 * only what somebody chose to start and stop. The prompt fields back the
 * 30-minute "Are you working?" check, which exists so a forgotten timer stops
 * accruing hours rather than to police anyone.
 */
export const timeEntryWorkTable = pgTable("time_entry_work", {
  id: text("id")
    .$defaultFn(() => createId())
    .primaryKey(),
  userId: text("user_id")
  ...
  lastPromptAt: timestamp("last_prompt_at", { mode: "date" }),
  promptMisses: integer("prompt_misses").default(0).notNull(),

  /**
   * Set when the clock was auto-paused for an unanswered prompt. Distinct from
   * endedAt: a paused entry is today's work in progress, an ended one is
   * finished, and collapsing the two would make a penalised morning read as a
   * short day.
   */
  pausedAt: timestamp("paused_at", { mode: "date" }),
  /** Seconds deducted by unanswered prompts. Subtracted from the day's total. */
  penaltySeconds: integer("penalty_seconds").default(0).notNull(),
  /**
   * When the CLIENT last actually rendered the "are you working?" card.
   * Distinct from lastPromptAt (when it was last ANSWERED): a penalty needs
   * evidence the person was asked, and a closed tab never asks.
   */
  promptShownAt: timestamp("prompt_shown_at", { mode: "date" }),
  /**
   * Last time the browser polled /work-time/me while this entry was open.
   * Server-observed proof the app was in front of them — unlike promptShownAt,
   * which the client reports and could simply decline to send.
   */
  lastSeenAt: timestamp("last_seen_at", { mode: "date" }),
  /** Admin who typed this slot in by hand; null for self-tracked sessions. */
  createdBy: text("created_by"),
  /** Payroll evidence: a hand-entered slot must never look self-tracked. */
  isManual: boolean("is_manual").default(false).notNull(),
  note: text("note"),
  /**
   * The task this session is currently attributed to, if any.
   *
   * Nullable on purpose. Clocking in without naming a task is still valid —
   * a timer you cannot start without filling in a form is a timer people stop
   * starting. This makes attribution possible, not mandatory.
   */
  currentTaskId: text("current_task_id").references(() => taskTable.id, {
    onDelete: "set null",
  }),
});

export const time_entry_work = timeEntryWorkTable;

/**
 * Per-project assignment (client meeting 2026-07-28).
 *
 * Workspace membership used to imply access to every board. VK wants each
 * employee to open the app and see only their own: "the moment he logs in, he
 * should just get this... at all times he will have one board."
 *
 * Owners and admins are intentionally absent — they see everything by role. So
 * an EMPTY table means "nobody is restricted", not "nobody has access", and
 * assigning the first person cannot lock the owner out of their own workspace.
 */
 ...
 * Files on a task.
 *
 * A real structure rather than markdown in the description, so the task detail
 * can render an Attachments panel the way Trello does — file-type icon, size,
 * date — instead of a list of links buried under the card's body text.
 *
 * storageKey points at MinIO for files we host; sourceUrl keeps the original
 * link so provenance survives for anything not mirrored.
 */
...
export const task_attachment = taskAttachmentTable;

export const workspace_member = workspaceUserTable;
export const user_access = userAccessTable;
