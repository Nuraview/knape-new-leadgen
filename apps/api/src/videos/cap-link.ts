// Client-safe Cap link utilities (no sharp/@vercel/blob imports — usable in
// browser components). Server-side resolution lives in cap-embed.ts, which
// re-exports these.
...
export function extractCapVideoId(input: string): string | null {
  ...
  // the group when it matched, but be explicit rather than assert.
  return match[1] ?? null;
}
