...
export async function migrateSessionColumn() {
  ...
  try {
     ...
     *
     * NOTE the `table_schema = 'public'` filter on every check in this file.
     * information_schema.tables spans EVERY schema the role can see, while the
     * ALTER statements below resolve against the search_path — so an unfiltered
     * existence check can pass on a table in another schema and then fail on a
     * table that does not exist here.
     *
     * Not hypothetical: provisioning Postgres through the Vercel Neon
     * integration also installs Neon Auth, which creates a `neon_auth` schema
     * containing its own `invitation` table. The unfiltered check matched that,
     * and bootstrapping an empty database died with
     * `relation "invitation" does not exist`.
     */
    console.log(
