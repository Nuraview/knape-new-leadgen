...
import { notifyComment } from "../../notification/whatsapp";
...
async function createComment(taskId: string, userId: string, content: string) {
  ...
  // Best effort, deliberately not awaited: WhatsApp delivery must never
  // delay or fail the comment the user just posted.
  if (task) {
    void notifyComment({
