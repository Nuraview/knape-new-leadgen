...
import { humanizeStatus, notifyOwnerWhatsapp } from "./whatsapp";
...
subscribeToEvent<{
  ...
  /*
   * EVERY card move goes to the owner's WhatsApp, unconditionally (meeting
   * 2026-07-30, VK: "whenever a person moves the card, I want to be on top of
   * my game" — he explicitly accepted the volume). Separate from the assignee
   * notification below, which only fires when the mover isn't the assignee.
   */
  void (async () => {
    ...
    await notifyOwnerWhatsapp(
      [
        `📋 *${data.title}*`,
        `${humanizeStatus(data.oldStatus)} → ${humanizeStatus(data.newStatus)}`,
        `by ${actor?.name || actor?.email || "someone"}`,
      ].join("\n"),
      "nuraview-app:card-move",
    );
  })().catch(() => {});
