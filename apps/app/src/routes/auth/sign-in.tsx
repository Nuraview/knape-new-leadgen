...
function SignIn() {
  ...
  const getCallbackUrl = useCallback(() => {
    // VITE_CLIENT_URL is empty in the shipped image so one bundle serves any
    // host; fall back to the origin actually being browsed. Without this the
    // OAuth callback became the string "undefined/dashboard".
  ...
  const handleCustomOAuth = useCallback(async () => {
    ...
    try {
      const result = await authClient.signIn.oauth2({
        ...
        errorCallbackURL: `${import.meta.env.VITE_CLIENT_URL || window.location.origin}/auth/sign-in`,
      });
      if (result.error) {
  ...
  const handleSignInSuccess = async () => {
    ...
    if (redirectPath) {
      ...
      // Landing page depends on entitlement. Sending everyone to /leads meant a
      // projects-only employee signed in and immediately hit a 403 — the API
      // refuses them, correctly, but the app had walked them into it.
      navigate({ to: await getLandingPath() });
    }
  };
          ...
          {(config?.hasGoogleSignIn ||
            ...
            <>
              <div className="space-y-3">
                ...
                {/*
                  "Continue as guest" removed. It called the anonymous()
                  plugin, which minted a real user row without an invitation —
                  the thing DISABLE_REGISTRATION exists to prevent. The plugin
                  is gone from apps/api/src/auth.ts, so the endpoint 404s now
                  too; this just stops offering a button that cannot work.

                  The Turnstile widget lived inside this block and went with
                  it. It was rendering the broken "Troubleshoot" link on the
                  sign-in page because VITE_TURNSTILE_SITE_KEY was set to the
                  literal placeholder "NURAVIEW_TURNSTILE_SITE_KEY".
                */}
              </div>
