...
export const EmbedBlock = Node.create({
  ...
  parseHTML() {
    return [
      {
        tag: "nuraview-embed[url]",
      },
      {
        tag: "div[data-type='nuraview-embed'][data-url]",
      },
    ];
  },
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    if (!isValidUrl(url)) {
      return [
        "div",
        attrs,
        ["span", { class: "nuraview-embed-invalid-link" }, url],
      ];
    }
    ...
    if (mode === "embed") {
      return [
        ...
        [
          "div",
          { class: "nuraview-embed-unsupported" },
  ...
  renderMarkdown(
    ...
    if (!isValidUrl(url)) {
      return `\n<span class="nuraview-embed-invalid-link">${escapeHtml(url)}</span>\n`;
    }
    return `\n<nuraview-embed url="${escapeHtml(url)}" mode="${mode}" />\n`;
  },
});
