...
function AttachmentCardView({ node }: NodeViewProps) {
  ...
  return (
    <NodeViewWrapper as="span" className="nuraview-attachment-node">
      <a
        href={url}
        target="_blank"
        rel="noopener noreferrer"
        className="nuraview-attachment-card"
        title={filename}
      >
        <span className="nuraview-attachment-card-icon">
          <FileText className="size-4" />
        </span>
        <span className="nuraview-attachment-card-content">
          <span className="nuraview-attachment-card-title">{filename}</span>
          <span className="nuraview-attachment-card-meta">
            {formatBytes(size)}
            {mimeType ? ` · ${mimeType}` : ""}
          </span>
        </span>
...
export const AttachmentCard = Node.create({
  ...
  renderMarkdown(
    ...
    return `\n<nuraview-attachment url="${url}" filename="${filename}" mime-type="${mimeType}" size="${size}" />\n`;
  },
});
