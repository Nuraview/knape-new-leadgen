 ...
 * calls /marketing/validate-email (format + MX + disposable + the self-hosted
 * Reacher SMTP probe) and renders the verdict badge plus a collapsible
 * breakdown of every sub-check.
 *
 * Ported from apps/web/components/marketing/deliverability-check.tsx. Only the
 * fetch changes (getApiUrl + credentials); the verdicts, the copy and the
 * check list are the legacy ones, because the send route enforces the same
 * verdict and 422s on "invalid" — the badge has to say what the server will do.
 */
...
export function DeliverabilityCheck({ email }: { email: string }) {
  ...
  const run = useCallback(async (addr: string) => {
    ...
    setChecking(true);
    try {
      const res = await fetch(getApiUrl("marketing/validate-email"), {
        method: "POST",
        credentials: "include",
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {result.score === "low" &&
                    "Low Deliverability — proceed with caution"}
...
function VerificationBreakdown({ result }: { result: Result }) {
  ...
  if (!hasData) {
    return (
      <p className="pl-6 text-xs italic text-muted-foreground">
        SMTP mailbox probe not available for this provider (Gmail/Outlook/Yahoo
        block it) — syntax + MX verified; bounces auto-caught after send.
      </p>
    );
  }
  ...
  const rows: {
    ...
    { label: "Syntax valid", state: yes(result.isValidFormat) },
    {
      ...
      state: yes(v.acceptsMail ?? result.hasMxRecords),
    },
    ...
    {
      label: "Not a role account",
      state: no(v.isRoleAccount),
      note: v.isRoleAccount ? "info@, support@…" : undefined,
    },
    { label: "Not disposable", state: no(v.isDisposable) },
    {
      label: "Gravatar profile",
      state: v.gravatarUrl ? "good" : "unknown",
      note: v.gravatarUrl ? "real person" : undefined,
    },
    {
      label: "Not in known breach (HIBP)",
      state: v.breached == null ? "unknown" : no(v.breached),
    },
  ];
