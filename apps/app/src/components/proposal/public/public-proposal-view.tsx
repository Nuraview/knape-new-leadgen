...
import { tierUnitPrice } from "@/lib/proposal-tiers";
import useBrand from "@/hooks/use-brand";

// Vite has no SSR here, so next/dynamic's ssr:false wrapper is unnecessary —
// a lazy import gives the same code-splitting without the framework.
...
interface Proposal {
  ...
  /** Agreed upfront share ("25% on signing"); null when billed in one go. */
  depositPercent?: number | null;
  /** Assumptions and exclusions, shown under the total. */
...
export function PublicProposalView({
  proposal, settings, token, alreadyDecided, pdfMode = false, initialPayment = null,
  payments = null,
}: {
  ...
  pdfMode?: boolean;
  /**
   * Payment state carried IN via the URL — the invoices.nuraview.com side of
   * the hand-off. Signing happens on proposals.nuraview.com; paying happens
   * here after a cross-host redirect, and a redirect keeps nothing in memory,
   * so the approve response's payment fields ride the query string instead.
   */
  initialPayment?: PaymentState | null;
  /** Publishable Stripe/PayPal ids, from the public payload (never env). */
  payments?: { stripePublishableKey: string | null; paypalClientId: string | null } | null;
}) {
  ...
  /*
   * `brand` above is a COLOUR, and has been since long before white-labelling
   * existed. The instance identity therefore needs its own name rather than the
   * obvious one. Safe on this page despite it being unauthenticated —
   * /api/config is public.
   */
  const instanceBrand = useBrand();
  ...
  const [decided, setDecided] = useState(alreadyDecided || Boolean(initialPayment));
  ...
  const totals = useMemo(() => {
    ...
    /*
     * Staged billing. The proposal agreed a percentage up front, so this is
     * what signing actually charges — the approve endpoint splits the same way.
     * Showing the full total next to a button that takes a quarter of it (or
     * worse, charging the full total, which is what happened before) is how a
     * signed deal turns into a refund request.
     */
    const depositPct = proposal.depositPercent ?? null;
    const dueNow =
      depositPct && depositPct > 0 && depositPct < 100
        ? Math.round(grand * (depositPct / 100) * 100) / 100
        : null;
    return {
      subtotal,
      tax,
  ...
  }, [quantities, proposal.lineItems, proposal.depositPercent, method]);
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        /* Neutralise pasted typography.
           The house terms were pasted from Google Docs, so they carry inline
           font-family:Georgia and font-size:11pt on almost every span. Those
           survive sanitising (the editor keeps author intent in storage) and
           made the terms render as cramped serif in the middle of a sans page.
           Inline styles only lose to !important, hence the hammer; the stat and
           spec tables below re-assert their own sizes the same way. Headings are
           left alone — they carry the display face deliberately. */
        .pv-prose :where(p,li,span,strong,em,a,div,td,th){
          font-family:inherit !important;
          font-size:inherit !important;
          line-height:inherit !important;
        }
        /* The reading measure lives on the TEXT, not on the container.
           Constraining the wrapper capped tables at 48rem while the pricing and
           scope cards ran the full section width, so every section had a
           different right edge. Prose stays comfortable to read; tables and
           figures line up with everything else. */
        .pv-measure > :where(p,ul,ol,h1,h2,h3,h4,blockquote,details){ max-width:48rem }
        .pv-prose :where(table){ border-collapse:collapse; width:100%; font-size:.95em }
        /* Horizontal rules only. A full grid of boxes reads like a spreadsheet
           dropped into a proposal; rules between rows carry the same
           information and let the type breathe. */
        .pv-prose :where(td,th){
          border:0; border-bottom:1px solid rgba(0,0,0,.08);
          padding:.75rem 1.25rem .75rem 0; vertical-align:top; text-align:left;
        }
        .pv-prose :where(tr:last-child td){ border-bottom:0 }
        .pv-prose :where(th){
          font-weight:600; font-size:.72rem !important; letter-spacing:.08em;
          text-transform:uppercase; color:#78716c;
          border-bottom:1px solid rgba(0,0,0,.14);
        }
        /* CKEditor wraps every table in <figure class="table">. In this app
           .table is ALSO a Tailwind utility meaning display:table, so the
           figure shrink-wrapped and the table's width:100% resolved against a
           box already collapsed to its content — a cramped little grid instead
           of a full-width band. Force the figure back to a block. */
        .pv-prose figure.table{ display:block; width:100%; margin:1.5rem 0 }
        .pv-prose figure.table > table{ width:100% }
        /* Headline figures: values on the first row, captions beneath. Reads as
           a stat band, so no cell borders — a rule above and below is enough. */
        .pv-prose figure.pv-stats > table{ border-top:1px solid rgba(0,0,0,.14); border-bottom:1px solid rgba(0,0,0,.14) }
        .pv-prose figure.pv-stats td{ border:none; padding:1rem 1.5rem 1rem 0; text-align:left }
        .pv-prose figure.pv-stats tr:first-child td{
          font-family:${displayFamily} !important; font-size:1.75rem !important;
          line-height:1.1 !important; font-weight:600; color:var(--pv-ink);
          padding-bottom:.15rem;
        }
        .pv-prose figure.pv-stats tr:last-child td{
          font-size:.7rem !important; letter-spacing:.09em;
          text-transform:uppercase; color:#78716c; padding-top:0;
        }
        /* Spec sheet: label column, value column, hairline between rows. */
        .pv-prose figure.pv-kv td{ border:none; border-bottom:1px solid rgba(0,0,0,.08); padding:.7rem 1rem .7rem 0 }
        .pv-prose figure.pv-kv tr:last-child td{ border-bottom:none }
        .pv-prose figure.pv-kv td:first-child{
          width:34%; color:#78716c; font-size:.78rem !important;
          letter-spacing:.06em; text-transform:uppercase; white-space:nowrap;
        }
        .pv-prose figure.pv-kv td:last-child{ color:var(--pv-ink); font-weight:500 }
        @media (max-width:640px){
          .pv-prose figure.pv-stats tr:first-child td{ font-size:1.25rem }
          .pv-prose figure.pv-stats td{ padding-right:.9rem }
          .pv-prose figure.pv-kv td:first-child{ width:42%; white-space:normal }
        }
        .pv-prose :where(img){ max-width:100%; height:auto; border-radius:.5rem }
        /* Collapsed cancellation terms. Everything the client agrees to stays
           on this page; the detail is one click away rather than three
           paragraphs of policy printed on a small fixed-price proposal. */
        .pv-prose :where(details){
          margin:1rem 0; border:1px solid rgba(0,0,0,.12); border-radius:.5rem;
          padding:.35rem .9rem; background:rgba(0,0,0,.015);
        }
        .pv-prose :where(details[open]){ padding-bottom:.5rem }
        .pv-prose :where(summary){
          cursor:pointer; list-style:none; padding:.5rem 0; font-weight:600;
          color:var(--pv-ink); user-select:none;
        }
        .pv-prose :where(summary)::-webkit-details-marker{ display:none }
        .pv-prose :where(summary)::after{
          content:"＋"; float:right; color:var(--pv-accent); font-weight:400;
        }
        .pv-prose :where(details[open] summary)::after{ content:"−" }
        .pv-prose :where(details p){ margin:.4rem 0; font-size:.95em }
        /* Printing must not hide terms behind a closed disclosure. */
        @media print { .pv-prose :where(details){ display:block }
          .pv-prose :where(details > *){ display:block !important } }
          ...
          <div className="flex items-center justify-between gap-4 py-6">
            <div className="flex items-center gap-3">
              {settings?.logoStorageKey ? (
                ...
                instanceBrand.name.trim().toLowerCase() ? (
                /*
                  The real wordmark, straight from the brand asset the website
                  uses. It is white-on-transparent and this bar is dark, so it
                  needs no treatment here. A company that is not THIS instance's
                  owner keeps the generic hexagon + name rather than wearing
                  somebody else's logo — which is why the comparison is against
                  the instance brand rather than the literal "nuraview" it used
                  to test for.
                */
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={instanceBrand.logoUrl}
                  alt={instanceBrand.name}
      ...
      <main className="relative mx-auto max-w-6xl px-6 sm:px-10">
        ...
        <section className="grid gap-8 py-14 lg:grid-cols-[1fr_360px] lg:items-center">
          ...
          return (
            <section key={s.key} className="border-t border-stone-200/70 py-14">
              ...
              {isScopeCards ? (
                ...
                s.bodyHtml && (
                  <div className="pv-prose pv-measure text-[16px] leading-[1.8] text-stone-600" dangerouslySetInnerHTML={{ __html: s.bodyHtml }} />
                )
              )}
            </section>
          );
          ...
          <section className="border-t border-stone-200/70 py-14">
            ...
            <div className="overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-[0_24px_60px_-40px_rgba(0,0,0,.4)]">
              ...
              <div className="pv-avoid border-t border-stone-100 bg-stone-50/60 px-6 py-6">
                  ...
                  <>
                    ...
                    <div className="mt-3 flex items-baseline justify-between border-t border-stone-200 pt-4">
                      <span className="text-xs uppercase tracking-[0.25em] text-stone-400">
                        Due on signing ({totals.depositPct}%)
                      </span>
                ...
                {/* Assumptions and exclusions, next to the number they
                    qualify. The model is asked for these and they were being
                    stored and never rendered, so a client agreed to a price
                    without seeing what it assumed. */}
                {proposal.publicNotes?.trim() && (
                  <p className="mt-5 border-t border-stone-200 pt-4 text-[13.5px] leading-[1.7] text-stone-500">
        ...
        {terms && (terms.bodyHtml?.trim() || terms.bannerUrl) && (
          <section className="border-t border-stone-200/70 py-14">
            ...
            {terms.bodyHtml && (
              <div className="pv-prose pv-measure text-[15px] leading-[1.7] text-stone-600" dangerouslySetInnerHTML={{ __html: terms.bodyHtml }} />
            )}
          </section>
        )}
        ...
        <section id="approve" className="scroll-mt-6 border-t border-stone-200/70 py-14">
          ...
          {pdfMode ? (
            ...
            <ApprovalPanel token={token} brand={brand} quantities={quantities} clientTimeline={clientTimeline}
              ...
              onApproved={(p: PaymentState | null) => {
                /*
                 * Signed → send them to the INVOICE.
                 *
                 * VK: "why show proposal again in invoices — show the actual
                 * invoice." This used to re-render the entire proposal on the
                 * invoices host with the Stripe secret and amount smuggled
                 * through the query string, so a client who had just signed was
                 * shown the sales document a second time, hero and Review &
                 * Sign button included. The invoice page owns its own totals and
                 * mints its own PaymentIntent from the database, so there is
                 * nothing to carry across.
                 *
                 * No invoice (conversion failed, nothing to charge) → stay here
                 * and use the in-page panel, which is still correct.
                 */
                if (p?.invoiceUrl) {
                  window.location.href = p.invoiceUrl;
                  return;
                }
                setDecided(true);
                if (p) setPayment(p);
