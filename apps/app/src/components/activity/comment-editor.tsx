...
import { NuraViewIssueLink } from "@/components/task/extensions/nuraview-issue-link";
import { NuraViewMention } from "@/components/task/extensions/nuraview-mention";
...
export default function CommentEditor({
  ...
  const editor = useEditor(
    {
      ...
      extensions: [
        StarterKit.configure({
          ...
          trailingNode: false,
          codeBlock: {
            HTMLAttributes: { class: "nuraview-tiptap-codeblock" },
          },
        }),
        Link.configure({
          autolink: true,
      ...
      editorProps: {
        attributes: {
          class: cn(
            proseClassName || "nuraview-comment-editor-prose",
            readOnly && "nuraview-comment-editor-prose-readonly",
          ),
        },
          ...
          if (issueKey || taskIdFromUrl) {
            ...
            view.dispatch(
              view.state.tr.replaceSelectionWith(
                view.state.schema.nodes.nuraviewIssueLink.create({
                  url,
                  issueKey: issueKey || "",
  ...
  useEffect(() => {
    ...
    const handleImagePreviewClick = (event: MouseEvent) => {
      ...
      if (!target.classList.contains("nuraview-editor-image")) return;
  ...
  const handleEditorMouseMove = useCallback(
    (event: ReactMouseEvent<HTMLElement>) => {
      ...
      if (target.closest(".nuraview-codeblock-language")) return;
      const hovered = target.closest(
        "pre.nuraview-tiptap-codeblock",
      ) as HTMLElement | null;
  ...
  const handleEditorMouseLeave = useCallback(
    (event: ReactMouseEvent<HTMLElement>) => {
      ...
      if (relatedTarget?.closest(".nuraview-codeblock-language")) return;
      ...
      codeLanguageHideTimeoutRef.current = window.setTimeout(() => {
        ...
        const pickerIsHovered = Boolean(
          document.querySelector(".nuraview-codeblock-language:hover"),
        );
  ...
  return (
    <section
      ...
      className={cn(
        "nuraview-comment-editor-shell",
        isDragActive && "is-drag-active",
        readOnly && "nuraview-comment-editor-shell-readonly",
        className,
      )}
    ...
    >
      ...
      {editor && hoveredCodeBlock && !disabled && (
        <div
          className="nuraview-codeblock-language"
          style={{
        ...
        >
          <button
            type="button"
            className="nuraview-codeblock-language-trigger nuraview-codeblock-copy-trigger"
            aria-label={
              isCodeCopied
          ...
          {!readOnly && (
            ...
            >
              <DropdownMenuTrigger asChild>
                <button
                  type="button"
                  className="nuraview-codeblock-language-trigger"
                >
      ...
      {editor && !readOnly && !disabled && showBubbleMenu && (
        <BubbleMenu
          editor={editor}
          className="nuraview-comment-editor-bubble"
        ...
        >
          <Button
            ...
            size="xs"
            className={cn(
              "nuraview-comment-editor-bubble-btn",
          ...
          <Button
            ...
            variant="ghost"
            size="xs"
            className="nuraview-comment-editor-bubble-btn"
      ...
      {editor && !readOnly && !disabled && showBubbleMenu && (
        <BubbleMenu
          editor={editor}
          pluginKey="nuraview-comment-table-bubble"
          className="nuraview-comment-editor-bubble"
          shouldShow={({ editor: activeEditor, from, to }) =>
        ...
        >
          ...
          <span className="nuraview-tiptap-bubble-separator" />
          <Button
            type="button"
            variant="ghost"
            size="xs"
      ...
      {slashMenu && !readOnly && !disabled && (
        <div
          className="nuraview-tiptap-slash-menu"
          style={{
            top: slashMenu.top,
            left: slashMenu.left,
            position: slashMenuPosition,
        ...
        >
          {filteredSlashCommands.length > 0 ? (
            groupedSlashCommands.map((group) => {
              ...
              return (
                <div key={group.title} className="nuraview-tiptap-slash-group">
                  <div className="nuraview-tiptap-slash-group-title">
                    {group.title}
                  </div>
                  {group.items.map((command) => {
                    ...
                    return (
                      ...
                      >
                        <span className="nuraview-tiptap-slash-label">
                          {command.label}
                        </span>
                        {command.shortcut && (
                          <span className="nuraview-tiptap-slash-shortcut">
                            {command.shortcut}
                          </span>
                        )}
                      </button>
            ...
            <div className="nuraview-tiptap-slash-empty">
      ...
      {editor && embedComposer && (
        <div
          className="nuraview-embed-composer"
          style={{
        ...
        >
          {embedComposer.mode === "choice" ? (
            <div className="nuraview-embed-choice-menu">
              <button
                type="button"
                className="nuraview-embed-choice-item is-primary"
                onMouseDown={(event) => {
                  event.preventDefault();
                  submitEmbedComposer("embed");
                }}
              >
                ...
                <span className="nuraview-embed-choice-hint">
                  {t("activity:comment.editor.hintTab")}
                </span>
              </button>
              <button
                type="button"
                className="nuraview-embed-choice-item"
                onMouseDown={(event) => {
                  event.preventDefault();
                  setEmbedComposer(null);
                  setEmbedComposerError(null);
            ...
            <form
              className="nuraview-embed-composer-form"
              onSubmit={(event) => {
                event.preventDefault();
                submitEmbedComposer("embed");
              }}
            >
              <Input
                ...
                autoFocus
              />
              <div className="nuraview-embed-composer-actions">
                <Button
                  type="button"
                  size="xs"
                  variant="ghost"
              ...
              {embedComposerError && (
                <p className="nuraview-embed-composer-error">
      ...
      <EditorContent
        editor={editor}
        className={cn("nuraview-comment-editor-content", contentClassName)}
      ...
      {!readOnly && !disabled && showQuickAttachButton && (
        <button
          type="button"
          className="nuraview-editor-quick-attach"
          onMouseDown={(event) => {
            event.preventDefault();
          }}
          onClick={() => openImagePicker(editor)}
      ...
      {isDragActive && (
        <div className="nuraview-editor-drop-indicator">
