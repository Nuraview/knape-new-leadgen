/**
 * AI email generator + review dialog for a lead.
 *
 * Ported from crmx1's apps/web/app/(routes)/crm/leads/[leadId]/components/
 * GenerateEmailButton.tsx, kept 1:1 — the two setups, the follow-up cadence
 * and threading rules, the review dialog's field order, the deliverability
 * badge, the Cap video card, the signature preview and the Cmd/Ctrl+Enter
 * send. This is the screen the outreach team works in all day; anything
 * "improved" here is something they have to relearn.
 *
 * Only the plumbing differs from the original, because the stack does:
 *   - fetch goes through getApiUrl() with credentials, not Next's same-origin
 *     /api routes,
 *   - CKEditor is React.lazy instead of next/dynamic,
 *   - endpoints are POST /lead/generate-email and POST /lead/:id/send-email.
 */
import {
  Check,
  ChevronDown,
...
// Keeps the 500KB+ editor bundle off the lead drawer's first paint.
const CkEditor = lazy(() =>
  import("@/components/marketing/ck-editor").then((mod) => ({
    default: mod.CkEditor,
  })),
);

/**
 * Readable text for the preview CARDS (not the editor, which needs the HTML).
 *
 * The generator sometimes returns email1.body as HTML and the follow-ups as
 * plain text, and a restored draft is always HTML because that is what gets
 * persisted. Dropped into a <p> unchanged, the card printed literal
 * "<p>Hello team!</p>" markup at the reviewer.
 */
...
function EditorFallback() {
  return (
    <div className="flex min-h-[250px] items-center justify-center rounded-lg border border-input bg-card px-4 py-3 text-muted-foreground">
      Loading editor...
    </div>
  );
}
...
export function GenerateEmailButton({
  ...
  // "Send from" identity picker. Options come from /marketing/senders
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(getApiUrl("marketing/senders"), { credentials: "include" })
      .then((r) => (r.ok ? r.json() : []))
      .then((data) => {
        if (cancelled) return;
        // API shape is { items: [{id,label}, ...] } — tolerate the legacy
        // { senders: [...] } and a bare array too.
  ...
  const isThreadedReply = (idx: number) => setupMode === "updated" && idx === 0;
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(getApiUrl("lead/generate-email"), {
        method: "POST",
        credentials: "include",
        headers: {
      ...
      if (Array.isArray(data.followups) && data.followups.length > 0) {
        const fus = (data.followups as { subject?: string; body?: string }[]).map(
          (f) => ({
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch(getApiUrl(`lead/${leadId}/send-email`), {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
      ...
      if (onSaveEmail) {
        ...
        // (the API normalizes empty → null for nullable text fields)
  ...
  if (!generatedEmail) {
    return (
      <div className="space-y-4">
        ...
        {/* Theme-aware, unlike the crmx1 original: a bare bg-red-50 card is a
            bright white-red slab in the dark panel. */}
        {error && (
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            ...
            <Button
              size="sm"
              variant="ghost"
              onClick={() => copyToClipboard(toPreviewText(generatedEmail.body), "body")}
            >
              {copied === "body" ? (
                <Check className="h-4 w-4" />
              ) : (
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground line-clamp-4 whitespace-pre-wrap">
              {toPreviewText(generatedEmail.body)}
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => copyToClipboard(toPreviewText(fu.body), "body")}
                >
                  <Copy className="h-4 w-4" />
                </Button>
              </CardHeader>
        ...
        >
          ...
          <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-muted-foreground">
                ...
                the end of the email as a clickable animated preview.
              </p>
            </div>
                ...
                <div>
                  ...
                  {followupPreviewOpen[idx] ? (
                    <div className="min-h-[200px] overflow-x-auto rounded-lg border border-input bg-white p-4 shadow-sm dark:bg-black/20">
                      <div className="prose prose-sm prose-slate max-w-none dark:prose-invert text-sm break-words [overflow-wrap:anywhere] [&_*]:max-w-full [&_img]:h-auto">
                        <div dangerouslySetInnerHTML={{ __html: fu.body }} />
                      </div>
                    </div>
                  ) : (
                    <div className="min-w-0 overflow-hidden [&_.ck-editor__editable]:break-words [&_.ck-editor__editable_*]:max-w-full">
          ...
          <div className="flex gap-2 border-t px-6 py-4">
            ...
            <Button
              onClick={handleSend}
              disabled={
                sendingEmail || !toEmail?.trim() || !editableSubject?.trim()
              }
              className="flex-1"
            >
              {sendingEmail ? (
