...
Integrate the fire-enrich multi-agent enrichment engine into NuraviewCRM so users can automatically populate contact fields (company info, LinkedIn, industry, funding stage, tech stack, etc.) using Firecrawl + GPT-4o. Supports both single-contact interactive enrichment and bulk background enrichment.
...
Drawer disabled + tooltip shown if contact has no email: _"Add an email to enable enrichment."_
