...
export async function GET(request: Request) {
  try {
    const interExtrabold = fs.readFileSync(path.resolve('./public/fonts/Inter-Bold.ttf'));
    ...
    const title = hasTitle ? searchParams.get('title')?.slice(0, 200) : 'NuraviewCRM';

    const hasDescription = searchParams.has('description');

    const description = hasDescription
      ? searchParams.get('description')?.slice(0, 200)
      : 'NuraviewCRM is an open source CRM build on top of NextJS. Technology stack: NextJS with Typescrtipt, MongoDB, TailwindCSS, React, Prisma, shadCN, resend.com, react.email and more. ';
