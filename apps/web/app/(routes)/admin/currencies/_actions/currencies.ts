...
import { ExchangeRateSource } from "@/lib/db-types";
...
export async function getCurrencies(): Promise<CurrencyValue[]> {
  const currencies = await orm.currency.findMany({ orderBy: { code: "asc" } });
...
export async function getExchangeRatesAdmin(): Promise<ExchangeRateValue[]> {
  const rates = await orm.exchangeRate.findMany({
...
export async function createCurrency(data: { code: string; name: string; symbol: string }) {
  ...
  await orm.currency.create({ data: { ...parsed, isEnabled: true, isDefault: false } });
  revalidatePath("/", "layout");
}

export async function toggleCurrency(code: string, isEnabled: boolean) {
  await orm.currency.update({ where: { code }, data: { isEnabled } });
  revalidatePath("/", "layout");
}

export async function setDefaultCurrency(code: string) {
  await orm.$transaction([
    orm.currency.updateMany({ data: { isDefault: false } }),
    orm.currency.update({ where: { code }, data: { isDefault: true, isEnabled: true } }),
    orm.crm_SystemSettings.upsert({
...
export async function getEcbAutoUpdate(): Promise<boolean> {
  const setting = await orm.crm_SystemSettings.findUnique({
...
export async function setEcbAutoUpdate(enabled: boolean) {
  await orm.crm_SystemSettings.upsert({
    where: { key: "ecb_auto_update" },
