...
export const emailLinkCrm = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const email = await orm.email.findUnique({
      where: { id: emailId },
      select: {
        fromEmail: true,
        toRecipients: true,
    ...
    const linked = await step.run("match-and-link", async () => {
      const [contacts, accounts] = await Promise.all([
        orm.crm_Contacts.findMany({
          where: { email: { in: addresses } },
          select: { id: true },
        }),
        orm.crm_Accounts.findMany({
          where: { email: { in: addresses } },
          select: { id: true },
        }),
      ]);
      ...
      if (contactLinks.length > 0) {
        await orm.emailsToContacts.createMany({ data: contactLinks, skipDuplicates: true });
      }
      if (accountLinks.length > 0) {
        await orm.emailsToAccounts.createMany({ data: accountLinks, skipDuplicates: true });
      }
    ...
    if (linked > 0 && email.imapUid) {
      const emailAccount = await orm.emailAccount.findUnique({
