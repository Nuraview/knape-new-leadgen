...
export const campaignProcessFollowUp = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const [campaign, followUpStep] = await step.run("load-step", async () => {
      return Promise.all([
        orm.crm_campaigns.findUnique({ where: { id: campaignId }, select: { status: true } }),
        orm.crm_campaign_steps.findUnique({ where: { id: stepId } }),
      ]);
    });
    ...
    const step0 = await step.run("get-step-0", async () => {
      return orm.crm_campaign_steps.findFirst({ where: { campaign_id: campaignId, order: 0 } });
    });
    if (!step0) return { skipped: true, reason: "no step 0" };

    const eligibleTargets = await step.run("filter-recipients", async () => {
      return orm.crm_campaign_sends.findMany({
        where: {
          step_id: step0.id,
    ...
    const sendRecords = await step.run("create-send-records", async () => {
      await orm.crm_campaign_sends.createMany({
