...
export const embedBackfill = inngest.createFunction(
  ...
  async () => {
    const [accounts, contacts, leads, opportunities] = await Promise.all([
      orm.crm_Accounts.findMany({ select: { id: true } }),
      orm.crm_Contacts.findMany({ select: { id: true } }),
      orm.crm_Leads.findMany({ select: { id: true } }),
      orm.crm_Opportunities.findMany({ select: { id: true } }),
    ]);
