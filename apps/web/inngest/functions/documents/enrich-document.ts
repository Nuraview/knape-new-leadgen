...
// Lazy — avoids module-eval failure when OPENAI_API_KEY is unset at build time.
...
export const enrichDocument = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const document = await step.run("load-document", async () => {
      const doc = await orm.documents.findUnique({
        where: { id: documentId },
        select: {
          id: true,
          key: true,
    ...
    await step.run("generate-embedding", async () => {
      if (contentText.length <= MAX_SINGLE_EMBED_CHARS) {
        ...
        await orm.crm_Document_Chunks.deleteMany({
