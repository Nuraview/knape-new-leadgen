...
export const enrichTarget = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const enrichmentId = await step.run("ensure-enrichment-log", async () => {
      ...
      const row = await orm.crm_Target_Enrichment.create({
    ...
    if (!anthropicKey) {
      await orm.crm_Target_Enrichment.update({
    ...
    const recentEnrichment = await orm.crm_Target_Enrichment.findFirst({
    ...
    await step.run("apply-target-fields", async () => {
      ...
      if (Object.keys(updates).length > 0) {
        await orm.crm_Targets.update({ where: { id: targetId }, data: updates });
      }
    ...
    const contactIds = await step.run("upsert-contacts", async () => {
      ...
      for (const contact of agentOutput.contacts ?? []) {
        ...
        const upserted = await orm.crm_Target_Contact.upsert({
          where: whereKey as Parameters<typeof orm.crm_Target_Contact.upsert>[0]["where"],
          create: {
            targetId,
            name: contact.name,
            email: contact.email,
