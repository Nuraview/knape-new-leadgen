...
export const InvoiceEmail = ({ number, message, userLanguage }: InvoiceEmailProps) => {
  const isEn = userLanguage !== 'cz';

  const previewText = isEn ? `Invoice ${number}` : `Faktura ${number}`;
      ...
      <Tailwind>
        <Body className='bg-white my-auto mx-auto font-sans'>
          <Container className='border border-solid border-slate-300 rounded-md my-[40px] mx-auto p-[20px] w-[465px]'>
            ...
            <Text className='text-black text-sm leading-[24px]'>{message ?? defaultMessage}</Text>
            <Hr className='border border-solid border-[#eaeaea] my-[26px] mx-0 w-full' />
            <Section>
              <Text className='text-slate-500 text-muted-foreground text-xs leading-[24px]'>
                {isEn ? 'This email was sent from ' : 'Tento e-mail byl odeslan z '}
                <strong>{process.env.NEXT_PUBLIC_APP_NAME ?? 'NuraviewCRM'}</strong>
                {baseUrl ? ` (${baseUrl})` : ''}
              </Text>
            </Section>
          </Container>
