...
beforeAll(async () => {
  ...
  const existingUser = await orm.users.findFirst();
  if (existingUser) {
  ...
  let account = await orm.crm_Accounts.findFirst();
  if (!account) {
    account = await orm.crm_Accounts.create({
  ...
  const taxRate = await orm.invoice_TaxRates.findFirst({
    where: { active: true },
  });
  testTaxRateId = taxRate?.id;

  // Find or create a series + settings (required for issuing)
  let series = await orm.invoice_Series.findFirst();
  if (!series) {
    series = await orm.invoice_Series.create({
      data: {
        name: "Test Series",
  ...
  let settings = await orm.invoice_Settings.findFirst();
  if (!settings) {
    // Find a base currency
    let currency = await orm.currency.findFirst({
      where: { isEnabled: true },
    });
    if (!currency) {
      currency = await orm.currency.create({
        data: { code: "USD", name: "US Dollar", symbol: "$", isEnabled: true },
      });
    }
    await orm.invoice_Settings.create({
      data: {
...
afterAll(async () => {
  ...
  if (createdInvoiceIds.length > 0) {
    ...
    await orm.invoice_Payments.deleteMany({
      where: { invoiceId: { in: createdInvoiceIds } },
    });
    // Delete activity
    await orm.invoice_Activity.deleteMany({
      where: { invoiceId: { in: createdInvoiceIds } },
    });
    // Delete line items
    await orm.invoice_LineItems.deleteMany({
      where: { invoiceId: { in: createdInvoiceIds } },
    });
    // Delete invoices
    await orm.invoices.deleteMany({
...
/**
 * These are true integration tests: they hit a live Postgres. CI runs without
 * one, so they are skipped unless TEST_DATABASE_URL is set. To run them:
 *   TEST_DATABASE_URL=postgres://... npx jest __tests__/invoices/lifecycle
 */
const describeIntegration = process.env.TEST_DATABASE_URL ? describe : describe.skip;

describeIntegration("Invoice lifecycle", () => {
  ...
  it("issues a draft and assigns a number", async () => {
    ...
    const fresh = await orm.invoices.findUniqueOrThrow({
      where: { id: draft.id },
    });
  ...
  it("adds payments and transitions through PARTIALLY_PAID to PAID", async () => {
    ...
    let inv = await orm.invoices.findUniqueOrThrow({
      where: { id: draft.id },
    });
    ...
    inv = await orm.invoices.findUniqueOrThrow({
      where: { id: draft.id },
    });
  ...
  it("cancels a draft invoice", async () => {
    ...
    const inv = await orm.invoices.findUniqueOrThrow({
      where: { id: draft.id },
    });
