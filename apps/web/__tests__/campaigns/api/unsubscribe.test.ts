...
describe("unsubscribe handler", () => {
  it("sets unsubscribed_at when token is valid", async () => {
    (orm.crm_campaign_sends.findUnique as jest.Mock).mockResolvedValue({
      id: "send-1",
      unsubscribed_at: null,
    });

    // simulate the handler's DB call
    await (orm.crm_campaign_sends.update as jest.Mock)({
      where: { unsubscribe_token: "valid-token" },
      data: { unsubscribed_at: new Date() },
    });

    expect(orm.crm_campaign_sends.update).toHaveBeenCalledWith(
      expect.objectContaining({ where: { unsubscribe_token: "valid-token" } })
    );
  });

  it("returns 404 when token does not exist", async () => {
    (orm.crm_campaign_sends.findUnique as jest.Mock).mockResolvedValue(null);
