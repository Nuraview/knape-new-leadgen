...
describe("embed-email: hash-skip path", () => {
  it("skips embedding when content hash is unchanged", async () => {
    ...
    (orm.email.findUnique as jest.Mock).mockResolvedValue(mockEmail);
    (orm.emailEmbedding.findUnique as jest.Mock).mockResolvedValue({
      contentHash: "hash-abc",
    });

    const text = "subject body";
    const newHash = (computeContentHash as jest.Mock)(text);
    const existing = await orm.emailEmbedding.findUnique({ where: { emailId: "e1" }, select: { contentHash: true } });

    expect(existing?.contentHash).toBe(newHash);
    expect(orm.$executeRaw).not.toHaveBeenCalled();
  });
});
