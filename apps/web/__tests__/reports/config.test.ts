...
describe("report config actions", () => {
  ...
  describe("saveConfig", () => {
    it("creates a new config", async () => {
      ...
      (orm.crm_Report_Config.create as jest.Mock).mockResolvedValue(mockConfig);
      const result = await saveConfig({ name: "Q1 Sales", category: "sales", filters: {}, isShared: false });
      expect(orm.crm_Report_Config.create).toHaveBeenCalledWith({ data: expect.objectContaining({ name: "Q1 Sales", category: "sales", createdBy: "user-1" }) });
  ...
  describe("loadConfigs", () => {
    it("returns own and shared configs for category", async () => {
      ...
      (orm.crm_Report_Config.findMany as jest.Mock).mockResolvedValue(configs);
      const result = await loadConfigs("sales");
      expect(orm.crm_Report_Config.findMany).toHaveBeenCalledWith({
  ...
  describe("deleteConfig", () => {
    it("deletes config owned by user", async () => {
      (orm.crm_Report_Config.delete as jest.Mock).mockResolvedValue({});
      await deleteConfig("config-1");
      expect(orm.crm_Report_Config.delete).toHaveBeenCalledWith({ where: { id: "config-1", createdBy: "user-1" } });
    });
  });

  describe("toggleShare", () => {
    it("toggles isShared flag", async () => {
      (orm.crm_Report_Config.update as jest.Mock).mockResolvedValue({ isShared: true });
      await toggleShare("config-1", true);
      expect(orm.crm_Report_Config.update).toHaveBeenCalledWith({ where: { id: "config-1", createdBy: "user-1" }, data: { isShared: true } });
    });
  });
});
