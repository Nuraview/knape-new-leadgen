...
describe("sales report actions", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (orm.exchangeRate.findMany as jest.Mock).mockResolvedValue(mockRates);
  });

  describe("getRevenue", () => {
    it("sums budget of won opportunities in date range", async () => {
      (orm.crm_Opportunities.findMany as jest.Mock).mockResolvedValue([
  ...
  describe("getWinLossRate", () => {
    it("calculates win/loss counts", async () => {
      (orm.crm_Opportunities.count as jest.Mock)
