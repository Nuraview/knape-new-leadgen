...
describe("report schedule actions", () => {
  ...
  describe("createSchedule", () => {
    it("creates a schedule linked to a config", async () => {
      ...
      (orm.crm_Report_Schedule.create as jest.Mock).mockResolvedValue(mockSchedule);
      const result = await createSchedule({ reportConfigId: "config-1", cronExpression: "0 9 * * 1", recipients: ["alice@example.com"], format: "pdf" });
      expect(orm.crm_Report_Schedule.create).toHaveBeenCalledWith({ data: expect.objectContaining({ reportConfigId: "config-1", cronExpression: "0 9 * * 1", format: "pdf", createdBy: "user-1" }) });
  ...
  describe("listSchedules", () => {
    it("returns schedules for the current user", async () => {
      ...
      (orm.crm_Report_Schedule.findMany as jest.Mock).mockResolvedValue(schedules);
      const result = await listSchedules();
      expect(orm.crm_Report_Schedule.findMany).toHaveBeenCalledWith({ where: { createdBy: "user-1" }, include: { reportConfig: true }, orderBy: { createdAt: "desc" } });
  ...
  describe("deleteSchedule", () => {
    it("deletes schedule owned by user", async () => {
      (orm.crm_Report_Schedule.delete as jest.Mock).mockResolvedValue({});
      await deleteSchedule("sched-1");
      expect(orm.crm_Report_Schedule.delete).toHaveBeenCalledWith({ where: { id: "sched-1", createdBy: "user-1" } });
    });
  });
});
