...
const mockPrisma = orm as jest.Mocked<typeof orm>;
...
describe("listApiTokens", () => {
  it("returns tokens for user without tokenHash", async () => {
    ...
    // Mock findMany on orm.apiToken
    (orm.apiToken.findMany as jest.Mock) = jest.fn().mockResolvedValue(mockTokens);

    const result = await listApiTokens("user-123");
    expect(result).toEqual(mockTokens);
    expect(orm.apiToken.findMany).toHaveBeenCalledWith(
