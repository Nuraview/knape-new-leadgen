...
const mockFindFirst = orm.emailAccount.findFirst as jest.MockedFunction<typeof orm.emailAccount.findFirst>;
...
describe("getEmailAccounts", () => {
  ...
  it("returns accounts for authenticated user", async () => {
    ...
    expect(orm.emailAccount.findMany).toHaveBeenCalledWith(
...
describe("createEmailAccount", () => {
  ...
  it("encrypts password and saves account for authenticated user", async () => {
    ...
    expect(orm.emailAccount.create).toHaveBeenCalledWith(
...
describe("deleteEmailAccount", () => {
  ...
  it("deletes account when ownership verified", async () => {
    ...
    expect(orm.emailAccount.delete).toHaveBeenCalledWith({ where: { id: "acc-1" } });
  });
});
...
describe("setEmailAccountActive", () => {
  ...
  it("updates isActive when ownership verified", async () => {
    ...
    expect(orm.emailAccount.update).toHaveBeenCalledWith({
      where: { id: "acc-1" },
      data: { isActive: false },
    });
  });
