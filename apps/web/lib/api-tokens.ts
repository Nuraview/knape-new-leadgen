...
export async function generateApiToken(
  ...
  const activeCount = await orm.apiToken.count({
    where: {
      userId,
      revokedAt: null,
  ...
  const created = await orm.apiToken.create({
    data: {
      name,
      tokenHash,
      tokenPrefix,
...
export async function validateApiToken(rawToken: string): Promise<string> {
  ...
  const token = await orm.apiToken.findUnique({
    where: { tokenHash },
  });
  ...
  void Promise.resolve(
    orm.apiToken.update({ where: { id: token.id }, data: { lastUsedAt: new Date() } })
  ).catch(() => {});
...
export async function revokeApiToken(
  ...
  const token = await orm.apiToken.findUnique({ where: { id: tokenId } });
  if (!token || token.userId !== userId) throw new Error("Not found");

  await orm.apiToken.update({
    where: { id: tokenId },
