...
export const crmTargetTools = [
  {
    ...
    async handler(args: { limit: number; offset: number }, userId: string) {
      ...
      const [data, total] = await Promise.all([
        orm.crm_Targets.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { created_on: "desc" },
        }),
        orm.crm_Targets.count({ where }),
      ]);
  ...
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      const target = await orm.crm_Targets.findFirst({
  ...
  },
  {
    ...
    async handler(
      ...
      const target = await orm.crm_Targets.create({
  ...
  },
  {
    ...
    async handler(
      ...
      const existing = await orm.crm_Targets.findFirst({
        where: { id: args.id, created_by: userId, deletedAt: null },
      });
      if (!existing) notFound("Target");
      const { id, ...updateData } = args;
      const target = await orm.crm_Targets.update({
        where: { id },
