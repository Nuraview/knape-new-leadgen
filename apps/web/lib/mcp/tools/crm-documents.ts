...
export const crmDocumentTools = [
  {
    ...
    async handler(
      ...
      const [data, total] = await Promise.all([
        orm.documents.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { createdAt: "desc" },
        }),
        orm.documents.count({ where }),
      ]);
  ...
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      const doc = await orm.documents.findFirst({
  ...
  },
  {
    ...
    async handler(
      ...
      const doc = await orm.documents.create({
        data: {
  ...
  },
  {
    ...
    schema: z.object({ id: z.string().uuid() }),
    async handler(args: { id: string }, userId: string) {
      const existing = await orm.documents.findFirst({
        where: { id: args.id, created_by_user: userId, deletedAt: null },
      });
      if (!existing) notFound("Document");
      const doc = await orm.documents.update({
        where: { id: args.id },
