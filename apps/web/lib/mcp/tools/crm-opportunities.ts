...
export const crmOpportunityTools = [
  {
    ...
    async handler(args: { limit: number; offset: number }, userId: string) {
      ...
      const [data, total] = await Promise.all([
        orm.crm_Opportunities.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { createdAt: "desc" },
        }),
        orm.crm_Opportunities.count({ where }),
      ]);
  ...
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      const opp = await orm.crm_Opportunities.findFirst({
  ...
  },
  {
    ...
    async handler(
      ...
      const opp = await orm.crm_Opportunities.create({
        data: {
          v: 0,
          name,
          ...rest,
  ...
  },
  {
    ...
    async handler(
      ...
      const existing = await orm.crm_Opportunities.findFirst({
        where: { id: args.id, assigned_to: userId, deletedAt: null },
      });
      if (!existing) notFound("Opportunity");
      const { id, budget, expected_revenue, close_date, currency, ...rest } = args;
      const opp = await orm.crm_Opportunities.update({
        where: { id },
        data: {
          ...rest,
