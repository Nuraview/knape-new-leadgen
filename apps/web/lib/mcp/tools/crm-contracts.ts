...
export const crmContractTools = [
  ...
  },
  {
    ...
    async handler(args: { id: string }, _userId: string) {
      const contract = await orm.crm_Contracts.findFirst({
  ...
  },
  {
    ...
    async handler(args: Record<string, any>, userId: string) {
      ...
      const contract = await orm.crm_Contracts.create({
        data: {
          v: 0,
          ...contractData,
  ...
  },
  {
    ...
    async handler(args: Record<string, any>, userId: string) {
      const existing = await orm.crm_Contracts.findFirst({
        where: { id: args.id, deletedAt: null },
      });
      if (!existing) notFound("Contract");
      const { id, startDate, endDate, ...rest } = args;
      const contract = await orm.crm_Contracts.update({
        where: { id },
        data: {
          ...rest,
