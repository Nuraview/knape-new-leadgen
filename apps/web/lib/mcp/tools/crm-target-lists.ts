...
export const crmTargetListTools = [
  {
    ...
    async handler(args: { limit: number; offset: number }, _userId: string) {
      ...
      const [data, total] = await Promise.all([
        orm.crm_TargetLists.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { created_on: "desc" },
          include: { _count: { select: { targets: true } } },
        }),
        orm.crm_TargetLists.count({ where }),
      ]);
  ...
  },
  {
    ...
    async handler(
      ...
      const tl = await orm.crm_TargetLists.findFirst({
  ...
  },
  {
    ...
    async handler(args: { name: string; description?: string }, userId: string) {
      const tl = await orm.crm_TargetLists.create({
  ...
  },
  {
    ...
    async handler(
      ...
      const existing = await orm.crm_TargetLists.findFirst({
        where: { id: args.id, deletedAt: null },
      });
      if (!existing) notFound("TargetList");
      const { id, ...updateData } = args;
      const tl = await orm.crm_TargetLists.update({
        where: { id },
        data: updateData,
      });
      return itemResponse(tl);
  ...
  },
  {
    ...
    async handler(
      ...
      if (!tl) notFound("TargetList");
      await orm.targetsToTargetLists.createMany({
  ...
  },
  {
    ...
    async handler(
      ...
      await orm.targetsToTargetLists.deleteMany({
        where: {
          target_list_id: args.target_list_id,
