...
export const campaignTools = [
  // ── Campaigns CRUD ────────────────────────────────────
  {
    ...
    async handler(args: { status?: string; limit: number; offset: number }, _userId: string) {
      ...
      const [data, total] = await Promise.all([
        orm.crm_campaigns.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { created_on: "desc" },
          include: { _count: { select: { steps: true, sends: true } } },
        }),
        orm.crm_campaigns.count({ where }),
      ]);
  ...
  },
  {
    ...
    async handler(args: { id: string }, _userId: string) {
      const campaign = await orm.crm_campaigns.findFirst({
  ...
  },
  {
    ...
    async handler(
      ...
      const campaign = await orm.crm_campaigns.create({
        data: {
          v: 0,
          name: args.name,
  ...
  },
  {
    ...
    async handler(args: { id: string }, _userId: string) {
      ...
      await orm.crm_campaigns.update({ where: { id: args.id }, data: { status: "paused" } });
  ...
  },
  {
    ...
    async handler(args: { id: string }, _userId: string) {
      ...
      await orm.crm_campaigns.update({ where: { id: args.id }, data: { status: "sending" } });
      await inngest.send({ name: "campaigns/send-now", data: { campaignId: args.id } });
      return itemResponse({ id: args.id, status: "sending" });
    },
  },
  ...
  // ── Templates ─────────────────────────────────────────
  {
    ...
    async handler(args: { limit: number; offset: number }, _userId: string) {
      ...
      const [data, total] = await Promise.all([
        orm.crm_campaign_templates.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { created_on: "desc" },
        }),
        orm.crm_campaign_templates.count({ where }),
      ]);
      return listResponse(data, total, args.offset);
    },
  },
  {
    ...
    schema: z.object({ id: z.string().uuid() }),
    async handler(args: { id: string }, _userId: string) {
      const template = await orm.crm_campaign_templates.findFirst({ where: { id: args.id, deletedAt: null } });
  ...
  },
  {
    ...
    async handler(
      ...
      const template = await orm.crm_campaign_templates.create({
        data: {
          name: args.name,
          description: args.description,
  ...
  },
  {
    ...
    async handler(args: Record<string, any>, _userId: string) {
      const existing = await orm.crm_campaign_templates.findFirst({ where: { id: args.id, deletedAt: null } });
      if (!existing) notFound("Template");
      const { id, ...updateData } = args;
      const template = await orm.crm_campaign_templates.update({ where: { id }, data: updateData });
      return itemResponse(template);
    },
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      const existing = await orm.crm_campaign_templates.findFirst({ where: { id: args.id, deletedAt: null } });
      if (!existing) notFound("Template");
      await orm.crm_campaign_templates.update({
        where: { id: args.id },
        data: softDeleteData(userId),
      });
      return itemResponse({ id: args.id, deletedAt: new Date().toISOString() });
  ...
  // ── Steps ─────────────────────────────────────────────
  {
    ...
    async handler(
      ...
      if (!campaign) notFound("Campaign");
      const step = await orm.crm_campaign_steps.create({ data: args });
  ...
  },
  {
    ...
    async handler(args: Record<string, any>, _userId: string) {
      const existing = await orm.crm_campaign_steps.findUnique({ where: { id: args.id } });
      if (!existing) notFound("CampaignStep");
      const { id, ...updateData } = args;
      const step = await orm.crm_campaign_steps.update({ where: { id }, data: updateData });
      return itemResponse(step);
    },
  },
  {
    ...
    async handler(args: { id: string }, _userId: string) {
      const existing = await orm.crm_campaign_steps.findUnique({ where: { id: args.id } });
      if (!existing) notFound("CampaignStep");
      await orm.crm_campaign_steps.delete({ where: { id: args.id } });
  ...
  // ── Target List Assignment ────────────────────────────
  {
    ...
    async handler(args: { campaign_id: string; target_list_id: string }, _userId: string) {
      ...
      if (!campaign) notFound("Campaign");
      await orm.campaignToTargetLists.create({
  ...
  },
  {
    ...
    async handler(args: { campaign_id: string; target_list_id: string }, _userId: string) {
      await orm.campaignToTargetLists.delete({
        where: {
  ...
  // ── Stats ─────────────────────────────────────────────
  {
    ...
    async handler(args: { id: string }, _userId: string) {
      ...
      if (!campaign) notFound("Campaign");
      const sends = await orm.crm_campaign_sends.findMany({
