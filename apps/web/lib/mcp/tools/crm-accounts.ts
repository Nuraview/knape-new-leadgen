...
export const crmAccountTools = [
  ...
  },
  {
    ...
    async handler(
      ...
      const account = await orm.crm_Accounts.create({
        data: {
          v: 0,
          name,
          ...rest,
  ...
  },
  {
    ...
    async handler(
      ...
      const existing = await orm.crm_Accounts.findFirst({
        where: { id: args.id, assigned_to: userId, ...isNotDeleted() },
      });
      if (!existing) notFound("Account");
      const { id, ...updateData } = args;
      const account = await orm.crm_Accounts.update({
        where: { id },
