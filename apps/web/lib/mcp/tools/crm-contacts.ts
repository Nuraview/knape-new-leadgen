...
export const crmContactTools = [
  ...
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      const contact = await orm.crm_Contacts.findFirst({
  ...
  },
  {
    ...
    async handler(
      ...
      const contact = await orm.crm_Contacts.create({
        data: {
          v: 0,
          last_name,
          ...rest,
  ...
  },
  {
    ...
    async handler(
      ...
      const existing = await orm.crm_Contacts.findFirst({
        where: { id: args.id, assigned_to: userId, deletedAt: null },
      });
      if (!existing) notFound("Contact");
      const { id, ...updateData } = args;
      const contact = await orm.crm_Contacts.update({
        where: { id },
