...
export const crmProductTools = [
  ...
  },
  {
    ...
    async handler(args: { id: string }, _userId: string) {
      const product = await orm.crm_Products.findFirst({
  ...
  },
  {
    ...
    async handler(
      ...
      const product = await orm.crm_Products.create({
        data: {
          name: args.name,
  ...
  },
  {
    ...
    async handler(args: Record<string, any>, userId: string) {
      const existing = await orm.crm_Products.findFirst({
        where: { id: args.id, deletedAt: null },
      });
