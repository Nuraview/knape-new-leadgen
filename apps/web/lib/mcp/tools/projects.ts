...
export const projectTools = [
  ...
  },
  {
    ...
    async handler(
      ...
      const board = await orm.boards.create({
        data: {
          v: 0,
          title: args.title,
  ...
  },
  {
    ...
    async handler(args: Record<string, any>, userId: string) {
      const existing = await orm.boards.findFirst({
        where: { id: args.id, ...userBoardWhere(userId) },
      });
      if (!existing) notFound("Board");
      const { id, ...updateData } = args;
      const board = await orm.boards.update({
        where: { id },
  ...
  // ── Sections ──────────────────────────────────────────
  {
    ...
    async handler(args: { board: string; title: string }, userId: string) {
      ...
      if (!board) notFound("Board");
      const maxPos = await orm.sections.aggregate({
        where: { board: args.board },
        _max: { position: true },
      });
      const section = await orm.sections.create({
        data: {
          v: 0,
          board: args.board,
          title: args.title,
  ...
  },
  {
    ...
    async handler(args: { id: string; title?: string; position?: number }, _userId: string) {
      const existing = await orm.sections.findUnique({ where: { id: args.id } });
      if (!existing) notFound("Section");
      const { id, position, ...rest } = args;
      const section = await orm.sections.update({
        where: { id },
  ...
  },
  {
    ...
    async handler(args: { id: string }, _userId: string) {
      const section = await orm.sections.findUnique({
        where: { id: args.id },
      ...
      await orm.sections.delete({ where: { id: args.id } });
  ...
  },
  {
    ...
    async handler(
      ...
      const sec = await orm.sections.findUnique({ where: { id: args.section } });
      if (!sec) notFound("Section");
      const maxPos = await orm.tasks.aggregate({
  ...
  },
  {
    ...
    async handler(args: Record<string, any>, userId: string) {
      const existing = await orm.tasks.findUnique({ where: { id: args.id } });
      if (!existing) notFound("Task");
      const { id, dueDateAt, ...rest } = args;
      const task = await orm.tasks.update({
        where: { id },
        data: {
          ...rest,
  ...
  // ── Comments ──────────────────────────────────────────
  {
    ...
    async handler(args: { task: string; comment: string }, userId: string) {
      const existing = await orm.tasks.findUnique({ where: { id: args.task } });
      if (!existing) notFound("Task");
      const tc = await orm.tasksComments.create({
  ...
  },
  {
    ...
    async handler(args: { task: string; limit: number; offset: number }, _userId: string) {
      ...
      const [data, total] = await Promise.all([
        orm.tasksComments.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { createdAt: "desc" },
          include: { assigned_user: { select: { id: true, name: true } } },
        }),
        orm.tasksComments.count({ where }),
      ]);
      return listResponse(data, total, args.offset);
    },
  },
