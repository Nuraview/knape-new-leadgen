...
export const crmLeadTools = [
  {
    ...
    async handler(args: { limit: number; offset: number }, userId: string) {
      ...
      const [data, total] = await Promise.all([
        orm.crm_Leads.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { createdAt: "desc" },
        }),
        orm.crm_Leads.count({ where }),
      ]);
  ...
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      const lead = await orm.crm_Leads.findFirst({
  ...
  },
  {
    ...
    async handler(
      ...
      const lead = await orm.crm_Leads.create({
  ...
  },
  {
    ...
    async handler(
      ...
      const existing = await orm.crm_Leads.findFirst({
        where: { id: args.id, assigned_to: userId, deletedAt: null },
      });
      if (!existing) notFound("Lead");
      const { id, ...updateData } = args;
      const lead = await orm.crm_Leads.update({
        where: { id },
