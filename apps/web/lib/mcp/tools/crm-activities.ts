...
export const crmActivityTools = [
  {
    ...
    async handler(
      ...
      const [data, total] = await Promise.all([
        orm.crm_Activities.findMany({
          where,
          ...paginationArgs(args),
          orderBy: { date: "desc" },
          include: { links: true },
        }),
        orm.crm_Activities.count({ where }),
      ]);
  ...
  },
  {
    ...
    async handler(args: { id: string }, userId: string) {
      const activity = await orm.crm_Activities.findFirst({
  ...
  },
  {
    ...
    async handler(
      ...
      const activity = await orm.crm_Activities.create({
        data: {
          ...rest,
  ...
  },
  {
    ...
    async handler(
      ...
      const existing = await orm.crm_Activities.findFirst({
        where: { id: args.id, createdBy: userId, deletedAt: null },
      });
      if (!existing) notFound("Activity");
      const { id, date, status, ...rest } = args;
      const activity = await orm.crm_Activities.update({
        where: { id },
        data: {
          ...rest,
