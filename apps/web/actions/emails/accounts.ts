...
export async function getEmailAccounts() {
  ...
  return orm.emailAccount.findMany({
    where: { userId },
    select: {
      id: true,
      label: true,
...
export async function createEmailAccount(input: CreateInput) {
  ...
  return orm.emailAccount.create({
    data: {
      userId,
      label: input.label,
...
export async function deleteEmailAccount(id: string) {
  const userId = await requireSession();
  const account = await orm.emailAccount.findFirst({ where: { id, userId } });
  if (!account) throw new Error("Not found");
  await orm.emailAccount.delete({ where: { id } });
}
...
export async function setEmailAccountActive(id: string, isActive: boolean) {
  ...
  const account = await orm.emailAccount.findFirst({ where: { id, userId } });
  if (!account) throw new Error("Not found");
  return orm.emailAccount.update({ where: { id }, data: { isActive } });
}
