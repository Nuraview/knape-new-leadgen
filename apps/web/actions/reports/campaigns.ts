...
export async function getCampaignPerformance(filters: ReportFilters): Promise<{
  ...
  const sent = await orm.crm_campaign_sends.count({ where: dateFilter });
  const opened = await orm.crm_campaign_sends.count({ where: { ...dateFilter, opened_at: { not: null } } });
  const clicked = await orm.crm_campaign_sends.count({ where: { ...dateFilter, clicked_at: { not: null } } });
  return {
    sent, opened, clicked,
...
export async function getCampaignROI(filters: ReportFilters): Promise<ChartDataPoint[]> {
  const campaigns = await orm.crm_campaigns.findMany({
...
export async function getTargetListGrowth(filters: ReportFilters): Promise<ChartDataPoint[]> {
  const lists = await orm.crm_TargetLists.findMany({
    where: { created_on: { gte: filters.dateFrom, lte: filters.dateTo } },
