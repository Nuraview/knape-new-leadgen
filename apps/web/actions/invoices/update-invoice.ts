...
export async function updateInvoice(invoiceId: string, raw: unknown) {
  ...
  const existing = await orm.invoices.findUniqueOrThrow({
    where: { id: invoiceId },
  ...
  if (input.lineItems) {
    const taxRates = await orm.invoice_TaxRates.findMany({
      where: {
        id: {
          in: input.lineItems
            .map((l) => l.taxRateId)
    ...
    return orm.$transaction(async (tx: any) => {
  ...
  const updated = await orm.invoices.update({
    where: { id: invoiceId },
    data: {
      ...buildUpdateData(input),
      activity: {
