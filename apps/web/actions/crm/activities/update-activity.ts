...
export const updateActivity = async (data: {
  ...
  try {
    ...
    const existingLinks = await (orm as any).crm_ActivityLinks.findMany({
      where: { activityId: data.id },
    });

    const activity = await orm.$transaction(async (tx: any) => {
    ...
    for (const link of allLinks) {
      ...
      if (!seen.has(key)) {
        seen.add(key);
        revalidatePath(
          `/crm/${ENTITY_SLUGS[link.entityType] ?? `${link.entityType}s`}/${link.entityId}`,
          "page"
        );
      }
    }

    const fullActivity = await (orm as any).crm_Activities.findUnique({
