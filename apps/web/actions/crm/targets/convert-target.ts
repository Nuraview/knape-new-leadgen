...
export async function convertTarget(
  ...
  const target = await orm.crm_Targets.findFirst({ where: { id: targetId, deletedAt: null } });
  ...
  try {
    const [account, contact] = await orm.$transaction(async (tx: any) => {
    ...
    revalidatePath("/campaigns/targets", "page");
