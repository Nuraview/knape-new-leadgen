...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  try {
    const existing = await orm.crm_Products.findUnique({ where: { id } });
    if (!existing || existing.deletedAt) {
      return { error: "Product not found" };
    }

    if (updateData.sku && updateData.sku !== existing.sku) {
      const skuExists = await orm.crm_Products.findUnique({ where: { sku: updateData.sku } });
      if (skuExists) {
    ...
    const product = await orm.crm_Products.update({
      where: { id },
      data: {
    ...
    revalidatePath("/crm/products", "page");
    revalidatePath(`/crm/products/${id}`, "page");
