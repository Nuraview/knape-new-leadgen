...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  try {
    const contract = await orm.crm_Contracts.findUnique({
      where: { id: contractId },
    });
    ...
    const lineItem = await orm.crm_ContractLineItems.create({
      data: {
        contractId,
    ...
    const allLineItems = await orm.crm_ContractLineItems.findMany({
      where: { contractId },
    });
