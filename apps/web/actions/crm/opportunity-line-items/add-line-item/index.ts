...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  try {
    const opportunity = await orm.crm_Opportunities.findUnique({
    ...
    if (productId) {
      const product = await orm.crm_Products.findUnique({
        where: { id: productId },
      });
    ...
    const lineItem = await orm.crm_OpportunityLineItems.create({
      data: {
        opportunityId,
    ...
    const allLineItems = await orm.crm_OpportunityLineItems.findMany({
      where: { opportunityId },
    });
