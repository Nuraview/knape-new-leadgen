...
const handler = async (data: InputType): Promise<ReturnType> => {
  ...
  try {
    const product = await orm.crm_Products.findUnique({ where: { id: productId } });
    ...
    const existingAssignment = await orm.crm_AccountProducts.findFirst({
    ...
    const assignment = await orm.crm_AccountProducts.create({
      data: {
